...
function detectContentType(content) {
    ...
    const htmlRegex = /<(!DOCTYPE\s+)?html[^>]*>|<head[^>]*>|<body[^>]*>|<div[^>]*>|<p[^>]*>|<h[1-6][^>]*>/i;
...
const PARAM_DESCRIPTIONS = {
    ...
    timeout: 'Maximum time to wait for page load in milliseconds. Increase for slow-loading sites (e.g., 120000 for 2 minutes). Default: 60000 (1 minute)',
    maxChars: 'Maximum number of characters to return from the scraped content. Useful for limiting response size. Default: 100000',
    startIndex: 'Character position to start reading from. Use with maxChars for pagination through large documents (e.g., startIndex: 100000 to skip first 100k chars). Default: 0',
    resultHandling: 'How to handle scraped content and MCP Resources. Options: "saveOnly" (saves as linked resource, no content returned), "saveAndReturn" (saves as embedded resource and returns content - default), "returnOnly" (returns content without saving). Default: "saveAndReturn"',
    forceRescrape: 'Force a fresh scrape even if cached content exists for this URL. Useful when you know the content has changed. Default: false',
...
function startBaseUrlCrawl(url, clients) {
    if (!clients.firecrawl)
        return;
    if (typeof clients.firecrawl.startCrawl !== 'function')
        return;
    if (!shouldStartCrawl(url))
        return;
    ...
    Promise.resolve(clients.firecrawl.startCrawl(crawlConfig))
        .then((result) => {
        if (!result.success) {
            console.warn(`Firecrawl crawl failed for ${crawlConfig.url}: ${result.error || 'Unknown error'}`);
        }
    })
        .catch((error) => {
...
export function scrapeTool(_server, clientsFactory, strategyConfigFactory) {
    return {
        name: 'scrape',
...
4. Remembers successful strategies for future requests`,
        ...
        handler: async (args) => {
            try {
                ...
                const { url, maxChars, startIndex, timeout, forceRescrape, cleanScrape, resultHandling } = validatedArgs;
                ...
                if (resultHandling === 'saveOnly' || resultHandling === 'saveAndReturn') {
                    try {
                        ...
                        const contentMimeType = extractedContent || cleanedContent
                ...
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Failed to scrape ${args?.url || 'URL'}: ${error instanceof Error ? error.message : String(error)}`,
                        },
                    ],
                    isError: true,
                };
