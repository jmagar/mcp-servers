 ...
 */
export async function scrapeUniversal(clients, options) {
    const { url } = options;
 ...
 */
export async function scrapeWithSingleStrategy(clients, strategy, options) {
    const { url } = options;
    try {
        switch (strategy) {
            case 'native': {
 ...
 */
export async function scrapeWithStrategy(clients, configClient, options, explicitStrategy) {
    ...
    if (explicitStrategy) {
        ...
        logDebug('scrapeWithStrategy', `Explicit strategy '${explicitStrategy}' failed, falling back to universal approach`);
        ...
        if (universalResult.success && universalResult.source !== 'none') {
            try {
                ...
                await configClient.upsertEntry({
                    prefix: urlPattern,
                    default_strategy: universalResult.source,
    ...
    if (configuredStrategy) {
        ...
        logDebug('scrapeWithStrategy', `Configured strategy '${configuredStrategy}' failed, falling back to universal approach`);
    }
